...
import { z } from "zod";
import type { T3BridgeClient } from "../bridge.js";
...
export function resolveDeliveryMode(mode?: "after-current" | "immediate") {
  return mode ?? "after-current";
}

export function registerPhase1(server: McpServer, getClient: () => T3BridgeClient) {
  server.registerTool(
    "t3_status",
    {
      title: "T3 Code status",
      description:
        "Check every local and T3 Connect environment: which computers are reachable, whether " +
        "auth works, and a summary of projects/threads. Use this first if " +
        "anything else fails.",
      inputSchema: {},
    },
    async () => {
      try {
        ...
        const environmentStatus = await client.environmentStatuses();
        let auth = false;
        ...
        return jsonResult({
          ...
          authOk: auth,
          environments: environmentStatus.environments,
          ...(environmentStatus.discoveryError
            ? { t3ConnectDiscoveryError: environmentStatus.discoveryError }
            : {}),
          ...summary,
        });
      } catch (e) {
  ...
  server.registerTool(
    "list_environments",
    {
      ...
      description:
        "List every computer available through the local T3 Code server or T3 Connect. Shows " +
        "which environments are reachable and how many workspaces and threads each contains.",
      inputSchema: {
        refresh: z.boolean().optional().describe("Refresh T3 Connect discovery (default false)"),
      },
    },
    async (args) => {
      try {
        return jsonResult(await getClient().environmentStatuses(args.refresh ?? false));
      } catch (e) {
        return errorResult(e);
      }
    },
  ...
  server.registerTool(
    "list_projects",
    {
      ...
      description:
        "List projects (workspaces/repos) across this computer and every T3 Connect environment, " +
        "with the owning environment and per-project thread counts.",
      inputSchema: {},
    },
    async () => {
      try {
        ...
        const projects = shell.projects
          ...
          .map((p) => ({
            ...
            environment: p.environmentLabel ?? null,
    ...
    async (args) => {
      try {
        ...
        return jsonResult({
          ...
          environment: t.environmentLabel ?? null,
  ...
  server.registerTool(
    "send_message",
    {
      ...
      description:
        ...
        "current model, runtime mode, and interaction mode unless overridden. Messages queue " +
        "after the active turn by default. Use immediate only when you intentionally want to " +
        "steer an active turn. Confirms that T3 projected the message before returning; use wait_for_turn or " +
      ...
      inputSchema: {
        ...
        deliveryMode: z
          .enum(["after-current", "immediate"])
          .optional()
          .describe("Delivery behavior (default after-current; immediate may steer an active turn)"),
      },
    },
    async (args) => {
      try {
        ...
        const command = {
          ...
          commandId: newId(),
          threadId: t.id,
          message: { messageId, role: "user", text: args.message, attachments: [] },
        ...
        for (let attempt = 0; attempt < 20; attempt++) {
          const projected = await client.thread(t.id, { turnLimit: 3 });
          if ((projected.thread.messages ?? []).some((message) => message.id === messageId)) {
            verified = true;
            break;
          }
        ...
        return jsonResult({
          ...
          deliveryMode,
          note: verified
            ? "T3 accepted and projected the message. Use wait_for_turn or get_thread to read the reply."
            : "T3 accepted the command, but projection could not be confirmed within 5 seconds. Check get_thread before retrying to avoid a duplicate.",
        });
      } catch (e) {
        return errorResult(e);
      }
